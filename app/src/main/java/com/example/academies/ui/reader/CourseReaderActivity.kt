...
class CourseReaderActivity : AppCompatActivity(), CourseReaderCallback {
    ...
    private fun populateFragment() {
        ...
        if (!isLarge) {
            ...
            var fragmentContent = supportFragmentManager.findFragmentByTag(ModuleListFragment.TAG)
    ...
    private val initObserver: Observer<Resource<List<ModuleEntity>>> = Observer { modules ->
        if (modules != null) {
            when (modules.status) {
                ...
                Status.SUCCESS -> {
                    val dataModule: List<ModuleEntity>? = modules.data
                    if (dataModule != null && dataModule.isNotEmpty()) {

                        val firstModule = dataModule[0]
                        ...
                        if (!isFirstModuleRead) {
                            ...
                            val lastReadModuleId = getLastReadFromModules(dataModule)
